    ...
    public PreviousDayUtcDataLoadDelayHoursUserSetting(IConfiguration configuration)
    {
        ...
        if (!NumberOfHours.HasValue)
        {
            throw new InvalidOperationException(
                $"{nameof(PreviousDayUtcDataLoadDelayHoursUserSetting)} configuration is missing or invalid. Please specify a valid number between 0 and 23.");
        }

        if (NumberOfHours < 0 || NumberOfHours > 23)
        {
            throw new InvalidOperationException(
                $"{nameof(PreviousDayUtcDataLoadDelayHoursUserSetting)} must be between 0 and 23. Please check configuration.");
        }
    }
